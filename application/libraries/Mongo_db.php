...
Class Mongo_db {
	...
	*/
	...
	private function connect() {
		...
		try {
			...
			$url = 'mongodb://127.0.0.1:27017/ropstam_api';
			
			$this->connect = new MongoDB\Client($url);//$url
	...
	*/
	...
	//$this->mongo_db->count_collection($collection="", $where_filter="")
	public function count_collection($collection="", $where_filter=""){

		if($where_filter ==""){

			$where_filter		=	array("id" => array('$ne' => "")) ;
		...
		try
		{
			...
			$records		=	$collection->aggregate($pipeline);
			$get_records	=	iterator_to_array($records);
			$count 			= json_decode(json_encode($get_records),true);
	...
	//Function for mongodate format
	public function convertMongodateToreadable($mongodatetime)
	{
		$seconds		=	(string)$mongodatetime/ 1000;
		$datetime	=	date("Y-m-d G:i:s", $seconds);
		return $datetime;
	}
	//End converToMongodttime
